...
export default function Modal({ data }: { data: ModalData }) {
  const [open, setOpen] = useState(true);
  const cancelButtonRef = useRef(null);
  const { details, parentImage, parentName, classData } = data;
  return (
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start ">
                  <div>
                    ...
                    <Dialog.Title
                        as="h2"
                        className="font-bold text-[20px] xl:text-[32px] leading-[30px] xl:leading-[48px] text-gray-900 flex justify-between"
                      >
                      <Image src={CommaIcon.src} className='w-[24px] h-[18px]' />
                      ...
                      <div
                        ...
                        className="mx-auto w-[310px] md:w-[480px] pt-[32px] gap-5"
                      ><div className='font-medium text-sm  mx-4'>
                        {details}
                        </div>
                        <div className="mt-5 flex flex-row items-center gap-3">
                          <Image src={parentImage} alt='parent Image' className={'h-10 w-10 bg-no-repeat bg-cover rounded-full'} />
                          <div className='w-[83%]'>
                  ...
                  <div
                    className=" hidden absolute top-6 right-7 sm:flex justify-between"
                    onClick={() => setOpen(false)}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
