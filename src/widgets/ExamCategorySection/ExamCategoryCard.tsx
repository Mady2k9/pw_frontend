...
export default function ExamCategoryCard({
  ...
  return (
    <div
      className={'overflow-hidden flex group mb-auto border border-zinc-100 card-shadow animationFromBottom cursor-default hover:border-[#B7B7B7] transitionAll200 rounded p-4'}>
          ...
          <div className={'flex flex-wrap'} id={slug}>
            {
              (showMore ? exams : ExamsToDisplay).map((exam, index) => (
                <Link href={exam.slug || '#'} key={index}>
                  <Pill
                    className={'mr-2 mb-2 animationFromBottom md:text-[14px] text-[12px] !slide-in-from-bottom-2 hover:border-[#B7B7B7] fade-in text-[#3D3D3D]'}><span>{exam.name}</span></Pill>
                </Link>
              ))
            }
            {
              ...
              <Pill onClick={() => {
                ...
                setShowMore(!showMore);
                    ...
                    className={'mr-2 mb-2  md:text-[12px] text-[14px] animationFromBottom !slide-in-from-bottom-2 hover:border-[#B7B7B7] fade-in text-[#3D3D3D]'}><span>{!showMore ? 'More' : 'Less'}</span>
                <ChevronDownIcon
