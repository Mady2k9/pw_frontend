    ...
    <>
      {
        (showMore ? subjects : subjects.slice(0, 3)).map((subject, index) => {
          ...
          const showDate = formatDate(schedule.startTime) !== formatDate(schedule.endTime);
            ...
            <div className={'flex flex-col md:flex-row items-start md:items-center gap-2.5'}>
              <div
                ...
                <div className={'flex md:items-center flex-col md:flex-row items-start mt-1'}>
                  {showDate &&
                    <div
                      className={'text-sm text-lighter font-medium md:border-r md:pr-3 whitespace-nowrap h-full'}>
                      {formatDate(schedule.startTime)} to {formatDate(schedule.endTime)}
                    </div>
                  }
                  <div
                    className={cn('text-sm text-lighter font-medium w-[90px] min-w-[90px]', {
                      ' md:border-r': teachersName,
                      'md:pl-3 md:pr-3 ': showDate
      ...
      {subjects.length > 3 && <Button size={'lg'} onClick={() => setShowMore(!showMore)}
                                      variant={'outline'}>{showMore ? 'Show Less' : `+${subjects.length - 3} more subject${subjects.length - 3 > 1 ? 's' : ''}`}</Button>}
    </>
